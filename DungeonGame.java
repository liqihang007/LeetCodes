...
public class DungeonGame {
	...
	public int calculateMinimumHP(int[][] dungeon) {	//	hwo to make dp[i][j] at least 1
        int res=0;
        ...
        for(int i=m-2;i>=0;i--){
        	for(int j=n-2;j>=0;j--){
        		...
        		dp[i][j]=Math.max(1, Math.min(right,down));
        	}
        }
        return dp[0][0];
    }
	...
	public static void main(String[] args) {
		...
		System.out.println(new DungeonGame().calculateMinimumHP(A));
	}
