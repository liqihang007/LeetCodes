...
1. If an element in the deque and it is out of i-(k-1), we discard them. 
	just need to poll from the head, as we are using a deque and elements 
	are ordered as the sequence in the array
2. Now only those elements within [i-(k-1),i] are in the deque. 
	We then discard elements smaller than a[i] from the tail. 
	This is because if a[x] <a[i] and x<i, then a[x] has no chance to be 
	the "max" in [i-(k-1),i], or any other subsequent window: a[i] would always be a better candidate.
3. As a result elements in the deque are ordered in both sequence in array 
...
public class SlidingWindowMaximum {
	...
	public int[] maxSlidingWindow(int[] nums, int k) {
		...
		for (int i = 0; i < nums.length; i++) { 
		    ...
		    q.offer(i);
		    if (i >= k - 1){ // store the tmporal max
		        r[ri] = nums[q.peek()];
		        ri++;
		    }
		}
